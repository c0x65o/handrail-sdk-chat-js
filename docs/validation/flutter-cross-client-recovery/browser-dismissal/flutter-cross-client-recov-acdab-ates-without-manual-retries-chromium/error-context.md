...
Locator:  getByRole('menu', { name: 'Popup menu', exact: true })
Expected: hidden
Received: visible
Timeout:  15000ms
...
Call log:
  ...
  - waiting for getByRole('menu', { name: 'Popup menu', exact: true })
    34 × locator resolved to <flt-semantics role="menu" aria-label="Popup menu" id="flt-semantic-node-166" aria-owns="flt-semantic-node-168">…</flt-semantics>
...
```ts
  ...
  70 |     await flutter.getByRole('button', { name: 'Thread subscriptions', exact: true }).click();
  71 |     await expect(flutter.getByRole('menuitem', { name: 'Join', exact: true })).toBeEnabled();
  72 |     await expect(flutter.getByRole('menuitem', { name: 'Notifications: all', exact: true })).toBeEnabled();
  73 |     expect(await flutter.getByRole('menuitem', { name: 'Retry loading subscriptions', exact: true }).count()).toBe(0);
  74 |     await flutter.keyboard.press('Escape');
> 75 |     await expect(flutter.getByRole('menu', { name: 'Popup menu', exact: true })).toBeHidden();
     |                                                                                  ^ Error: expect(locator).toBeHidden() failed
  76 |     const status = await call(flutter, 'status');
  77 |     expect(status.provenance.sourceDigest).toBe(flutterLabProvenance().sourceDigest);
  78 |     expect(status.diagnostics).not.toContain('snapshot_hydration_failed');
  79 |     expect(errors).toEqual([]);
  80 |     await composer.getByRole('textbox').clear();
  81 |   } finally {
  82 |     await writeFile(info.outputPath('recovery.json'), JSON.stringify({ name, errors,
  83 |       semantics: await flutter.locator('body').ariaSnapshot(),
  84 |       exported: await call(flutter, 'export').catch(error => ({ error: error.message })) }, null, 2));
  85 |     await flutter.screenshot({ path: info.outputPath('recovery-390x844.png') });
